  ...
  tabsetPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          selectInput("esg_category", "Select ESG Category:",
            choices = c(
              "Environmental" = "environment_score",
              "Social" = "social_score",
      ...
      sidebarLayout(
        sidebarPanel(
          selectInput("industry_input", "Select Industry:",
            choices = unique(data4$industry),
            selected = unique(data4$industry)[1]
          ),
          helpText("Analyze ESG scores and categories for selected industries."),
    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          helpText(
            "This tool helps investors and stakeholders explore how companies perform in three key areas:",
            tags$strong("Environmental, Social, and Governance (ESG)"),
          ...
          selectInput("score_type", "Select Score Type:",
            choices = c(
              "Environmental Score" = "environment_score",
...
server <- function(input, output, session) {
  ...
  output$statisticsText <- renderPrint({
    filtered_data <- subset(data4, industry == input$industry_input)
  ...
  output$industrySummaryPlot <- renderPlot({
    ...
    ggplot(filtered_data, aes(x = reorder(name, -total_score), y = total_score, fill = grade_color)) +
      ...
      scale_fill_identity() +
      labs(
        title = paste("ESG Scores of Companies in", input$industry_input),
        x = "Company",
        y = "ESG Score",
  ...
  output$esgHeatmapPlot <- renderPlot({
    heatmap_data <- data4 %>%
      ...
      pivot_longer(
        cols = c(environment_score, social_score, governance_score),
    ...
    ggplot(heatmap_data, aes(x = criteria, y = name, fill = score)) +
      ...
      labs(
        title = paste("ESG Scores Heatmap for", input$industry_input),
  ...
  filtered_data <- reactive({
    data4 %>%
      ...
      select(c(
        name, environment_grade, social_grade, governance_grade, total_grade, environment_score,
  ...
  output$filteredData <- DT::renderDT({
    filtered_data() %>%
      rename(
        Company = name, `Environmental Grade` = environment_grade,
